...
import { palette, droneTypeLabel } from "../../src/theme";
...
export default function NewChecklist() {
  ...
  const handlePhotoFile = async (file: File) => {
    // First show the photo immediately so user sees it
    const reader = new FileReader();
    reader.onload = (e) => setUploadedPhoto(e.target?.result as string);
    reader.readAsDataURL(file);

    // Then auto-extract items from the image via backend
    setPhotoBusy(true);
    setPhotoError(null);
    try {
      const formData = new FormData();
      formData.append("file", file, file.name || "checklist.jpg");
      const { data } = await api.post("/checklists/parse-image", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      if (!data.items || data.items.length === 0) {
        setPhotoError("No checklist items found in this photo. Type them manually below.");
        setPhotoBusy(false);
        return;
      }
      setItems(data.items.map((it: any) => it.label));
      ...
      setPhotoError(formatApiError(e) || "Could not read this photo. Type items manually below.");
    } finally {
      setPhotoBusy(false);
    }
  };

  const rescanPhoto = async () => {
    if (!uploadedPhoto) return;
    setPhotoBusy(true);
    setPhotoError(null);
    try {
      const resp = await fetch(uploadedPhoto);
      const blob = await resp.blob();
      const file = new File([blob], "checklist.jpg", { type: blob.type || "image/jpeg" });
      const formData = new FormData();
      formData.append("file", file, file.name);
      const { data } = await api.post("/checklists/parse-image", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      ...
      if (data.items && data.items.length > 0) {
        ...
        setPhotoError("Still could not find items. Try typing them manually.");
      }
    } catch (e: any) {
      setPhotoError(formatApiError(e) || "Re-scan failed. Type items manually.");
    } finally {
      setPhotoBusy(false);
    }
  };
        ...
        {!uploadedPhoto ? (
          <>
            ...
            <Text style={[styles.stepDesc, { textAlign: "center" }]}>
              Take a photo or upload an image of your checklist.
              FlyReady will automatically read all items from it — just like PDF upload.
            </Text>
                ...
                <TouchableOpacity style={styles.uploadBtn} onPress={async () => {
                  ...
                  if (!r.canceled && r.assets?.[0]?.base64) {
                    const file = await (async () => {
                      const resp = await fetch(`data:image/jpeg;base64,${r.assets[0].base64}`);
                      const blob = await resp.blob();
                      return new File([blob], "checklist.jpg", { type: "image/jpeg" });
                    })();
          ...
          <View style={styles.busyCard}>
            <Image source={{ uri: uploadedPhoto }} style={[styles.previewImg, { opacity: 0.5 }]} resizeMode="contain" />
            <ActivityIndicator color={palette.primary} size="large" style={{ marginTop: 24 }} />
            <Text style={{ color: t.textSecondary, marginTop: 12, fontSize: 14 }}>Reading your checklist…</Text>
            <Text style={{ color: t.textSecondary, fontSize: 12, marginTop: 4 }}>This takes a few seconds</Text>
          </View>
        ) : (
          <>
            {photoError ? (
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              <TouchableOpacity onPress={() => { setUploadedPhoto(null); setPhotoError(null); setItems([""]); }} style={[styles.uploadBtn, { flex: 1, backgroundColor: t.surface, borderWidth: 1, borderColor: t.border }]}>
                <Feather name="refresh-cw" size={16} color={t.text} />
                <Text style={[styles.uploadBtnText, { color: t.text }]}>New photo</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={rescanPhoto} style={[styles.uploadBtn, { flex: 1, backgroundColor: palette.primary + "20", borderWidth: 1, borderColor: palette.primary }]}>
                <Ionicons name="scan-outline" size={16} color={palette.primary} />
                <Text style={[styles.uploadBtnText, { color: palette.primary }]}>Re-scan</Text>
              </TouchableOpacity>
            </View>

            {!photoError && items.some(i => i.trim()) && (
              <View style={styles.importBanner}>
                <Feather name="check-circle" size={15} color={palette.success} />
                <Text style={styles.importBannerText}>{items.filter(i => i.trim()).length} items extracted — edit below then save</Text>
              </View>
            )}

            <Text style={styles.sectionLabel}>Checklist items — edit if needed</Text>
            {items.map((item, idx) => (
            ...
            <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="e.g. Multirotor Preflight" placeholderTextColor={t.textSecondary} />
