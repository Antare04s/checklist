...
export default function Home() {
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]} testID="home-screen">
      ...
      <View style={styles.topBar}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <View style={styles.brandBadge}>
            ...
            <Ionicons name="checkmark" size={14} color={palette.success} style={{ position: "absolute", top: 9, left: 9 }} />
          </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }} refreshControl={<RefreshControl refreshing={refresh} onRefresh={onRefresh} />}>
        {/* QR scan card */}
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}><Text style={styles.statValue}>{stats.flights}</Text><Text style={styles.statLabel}>flights</Text></View>
          <View style={styles.statCard}><Text style={styles.statValue}>{stats.checklists}</Text><Text style={styles.statLabel}>checklists</Text></View>
          <View style={styles.statCard}><Text style={styles.statValue}>{stats.aircraft}</Text><Text style={styles.statLabel}>aircraft</Text></View>
        </View>
        ...
        {checklists.length === 0 ? (
          <View style={styles.empty}><Text style={styles.emptyText}>No checklists yet</Text></View>
        ) : (
          checklists.map((cl) => (
            <TouchableOpacity key={cl.id} testID={`checklist-card-${cl.id}`} style={styles.clCard} onPress={() => openChecklist(cl)}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.clMeta}>
                  {droneTypeLabel(cl.drone_type)} · {(cl.phase || "preflight").toString().replace("_", " ")}
                </Text>
      ...
      {/* Floating + button */}
      <TouchableOpacity testID="home-add-checklist-fab" style={styles.fab} onPress={() => router.push("/checklist/new")}>
