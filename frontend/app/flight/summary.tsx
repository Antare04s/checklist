          ...
          {failedItems.length > 0 && (
            <View style={styles.failedCard}>
              ...
              {failedItems.map((it: any) => (
                <Text key={it.id} style={styles.failedItem}>• {it.label}</Text>
              ))}
            </View>
          )}
          ...
          <View style={styles.chipsRow}>
            {SEVERITIES.map((s) => (
              <TouchableOpacity
                ...
                style={[styles.chip, draft.damage_severity === s.id && { backgroundColor: s.color, borderColor: s.color }]}
                onPress={() => draft.patch({ damage_severity: s.id as any })}
              >
          ...
          <View style={styles.mediaGrid}>
            {draft.media.map((m, i) => (
              <View key={i} style={styles.thumb}>
                ...
                <TouchableOpacity onPress={() => removeMedia(i)} style={styles.thumbX}>
                  <Feather name="x" size={14} color={palette.white} />
                </TouchableOpacity>
              </View>
            ))}
            <TouchableOpacity testID="sum-add-photo" style={styles.thumbAdd} onPress={pickPhoto}>
